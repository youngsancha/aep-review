...
- [ ] **P2** 기존 aep/allears 480여 회차의 _ko.json 이 60~70% 죽어 있음 — 과거 resegment 변경 누적으로 키가 어긋남(측정: ep100 243/413, ep400 347/478, ep520 272/393, v1.63.0 이전 기준). 온라인은 즉석 번역으로 가려지지만 오프라인에선 한국어가 아예 안 보임. 재번역 규모/비용 판단 필요
  ...
  - **2026-09-01 재측정 + 원인 확정 + 무료 경로 구축.** 전체 76.1% (273,134/358,754), 미번역 **85,620**
    (다운로드 실패 0편 = 완전 실측). aep 99.5% · allears 91.4% · wh 95.5% · **cnn10 1.7%**(395편 중
    387편에 `_ko.json` 자체가 없음, 미번역 72,306 = 전체의 84%).
  - ⭐ **'오프라인에서 번역이 안 뜬다'의 정확한 원인**: 앱은 `_ko.json` 에 키가 없으면 재생 중
    MyMemory 로 폴백하는데 오프라인에선 그게 실패해 "Translation needs a connection" 만 뜬다.
    쇼별 **오프라인 다운로드 창(최신 15편, R2 호스팅)** 커버리지를 재보면 원인이 한 곳에 몰려 있다:
    **aep 100% · allears 100% · wh 100% · cnn10 0.0%** (15편 전부 0%, 3,048문장). 즉 cnn10 을
    오프라인으로 들으면 **전 문장**이 번역 없이 뜬다. 다른 쇼는 멀쩡하다.
  - ⭐ **무료 백필 경로 추가**: `AEP_LLM_BACKEND=ollama` (로컬 exaone3.5:7.8b). API 과금 0,
    Claude 한도 0, 네트워크 0 — 8만 문장을 밤새 돌려도 아무것도 소모하지 않는다.
    ⛔ 대신 batch=1 에서만 정렬이 유지된다(실측: batch=1 → 21/24, batch=4 → 0/24). ~4.6s/문장.
  - ⛔⛔ **로컬 모델의 진짜 위험은 오역이 아니라 '밀림'이었다**: 32줄을 보내면 29개로 병합·번호
    재매김 → JSON 유효, 한국어 자연스러움, 문장 단위 검증 전부 통과, 그런데 **모든 문장이 옆 문장의
    번역과 짝지어진다.** `_ko.json` 은 멱등 채움이라 영구 저장된다. 앵커(원문 앞 4어절 되읊기)
    검증으로 막았다 — 현재 백필 로그 기준 정렬 실패 0건.
  - 진행 중: cnn10 오프라인 창 15편 백필(`~/Library/Logs/aep-translate-cnn10-offline.log`).
    남은 back catalogue(약 69,000문장)는 같은 명령에 `--show cnn10` 로 이어서 돌리면 된다.
...
## 2026-08-31 09:00 넘어옴

- [ ] **P2** E-Podcast Talk 모드 폰 실사용 검증 (S23 Chrome): 마이크 권한 / 첫-단어 지연이 현실적인지 / 상대 발화 TTS 자연스러운지
